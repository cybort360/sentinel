...
.PHONY: setup dev check check-integration build-sandbox demo web docker-build docker-up deploy help
...
dev: ## Run orchestrator + 3 MCP servers locally (TODO: implement)
	@echo "[dev] not yet implemented — see architecture.md §8."
...
demo: ## Run demo/run_demo.py end-to-end (needs build-sandbox running)
	uv run python -m demo.run_demo

web: ## Serve the War Room web UI at :8088 (needs build-sandbox running)
	uv run python -m demo.web_demo
